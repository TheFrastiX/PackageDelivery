    ...
    : EfRepository<Booking, Guid>(context), IBookingRepository
{
    ...
    public Task<IEnumerable<Booking>> GetByGuestIdAsync(Guid guestId, CancellationToken cancellationToken)
        => _bookings.Include(b => b.Guest)
        .Where(b => EF.Property<Guid>(b, "GuestId") == guestId)
        .ToListAsync(cancellationToken);

    public Task<IEnumerable<Booking>> GetByDateRangeAsync(DateTime checkIn, DateTime checkOut, CancellationToken cancellationToken)
        => _bookings.Include(b => b.Guest)
        .Where(b => b.DateRange.CheckIn <= checkOut && b.DateRange.CheckOut >= checkIn)
        .ToListAsync(cancellationToken);

    public Task<IEnumerable<Booking>> GetByStatusAsync(BookingStatus status, CancellationToken cancellationToken)
        => _bookings.Include(b => b.Guest)
        .Where(b => b.Status == status)
        .ToListAsync(cancellationToken);

    public Task<IEnumerable<Booking>> GetActiveBookingsAsync(CancellationToken cancellationToken)
        => _bookings.Include(b => b.Guest)
